...
public enum Terminal {
    ...
    public static var shouldApplyColors: Bool = {
        if let noColor = getenv("NO_COLOR") {
            String(cString: noColor).isEmpty
        } else {
            true
        }
    }()
    ...
    public static func print(_ string: StyledString, terminator: String = "\n", via stream: WritableStream = .output) {
        ...
        string.fragments.forEach { fragment in
            ...
            if stylesCanBeApplied
                && (fragment.style.foreground != nil || fragment.style.background != nil || fragment.style.weight != nil
    ...
    ///
    /// - Parameter stream: the stream being targeted.
    public static func print(via stream: WritableStream = .output) {
        unsafeWrite("\n", via: stream)
    }
    ...
    private static func setColor(_ color: Color, at layer: Layer, via stream: WritableStream) {
        switch color {
        case .ansi(let ansi):
            unsafeWrite("\u{1b}[\(layer.rawValue)8;5;\(ansi)m", via: stream)
        case .srgb(let rgb):
    ...
    public static var dimensions: Dimensions {
        get throws {
            ...
            if !isInputRedirected && ioctl(STDIN_FILENO, TIOCGWINSZ, &systemSize) == -1 && !isOutputRedirected
                && ioctl(STDOUT_FILENO, TIOCGWINSZ, &systemSize) == -1 && !isErrorRedirected
                && ioctl(STDERR_FILENO, TIOCGWINSZ, &systemSize) == -1
            {
    ...
    public struct Style {
        ...
        public init(
            foreground: Color? = nil, background: Color? = nil, weight: Weight? = nil, effects: Set<Effect>? = nil,
            padding: Padding? = nil
        ) {
